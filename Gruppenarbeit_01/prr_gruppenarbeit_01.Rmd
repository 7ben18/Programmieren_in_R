...
```{r}
hist(lm.fit3$residuals) # Ueberpreufen vom Erwartungswert =  0 + Normalverteilt
```
### Interpretation
...
```{r}
Carseats$Urban <- as.integer(as.character(Carseats$Urban)=="Yes")
str(Carseats)
```


```{r}
logreg.fit1 <- glm(Urban ∼ Sales, data = Carseats , family = binomial)
plot(logreg.fit1)
```

```{r}
Predicted_data <- data.frame(Sales=seq(min(Carseats$Sales), max(Carseats$Sales),len=500))
Predicted_data$Urban = predict(logreg.fit1, Predicted_data, type="response")
plot(Urban ~ Sales, data = Carseats)
lines(Urban ~ Sales, Predicted_data, lwd=2, col="green")
```
